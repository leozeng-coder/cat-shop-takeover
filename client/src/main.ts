...
app.addEventListener('click', async (event) => {
  ...
  if (op === 'mode') {
    ...
    el('modal').innerHTML =
      ...
      ' 秒准备。点击街道移动，猫猫会绕过墙和货架，从店门进入。</li><li>每局随机生成 8–10 间紧凑猫店，供 6 只猫选择；每店都有一个罐头窝，额外随机放 1–2 个道具。点击空店的窝，猫会走过去安家，谁先到达谁安家，选中猫窝不会提前占位。安家后立即关门，房主不能出门；屋内其他猫可开门出去，出去后不能再进，店长也不能进来。</li><li>点自家空格，选择弹射器、储藏柜或修补台。点已有道具、罐头窝或店门，可以升级和修补。</li><li>安家后持续赚罐头，数量显示在右上角。起身后收入保持不变；走到罐头箱上还能拾取物资。</li><li>白天店长回来：先敲门、破门，再进店追猫。店长会随时间、敲门和受击积累怒气值，升级时回复部分生命值，并向全体猫猫播报。左上角显示全员头像和店长等级、血量。店长正在敲门的猫店，其主人头像右下角会出现店长，每 2 秒撞击一次头像，提醒你及时防守。店门被打破后，猫猫自动起身，真人点击地图控制逃跑，AI 自动避让；此时无法安装、升级、修门或回窝。店长会走进房间，接触到猫才会把它抓走；猫没有生命值或扣血阶段。倒计时结束时仍有猫留守就获胜（整局 ' +
