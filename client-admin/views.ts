...
export function shell(page: string) {
  ...
  return `<div class="workbench">
    ...
    <div class="main">
      <header class="topbar"><div><div class="breadcrumb"><span>${active.module.name}</span><b>/</b>${active.page.group ? `<span>${active.module.groups?.find((g) => g.id === active.page.group)?.name}</span><b>/</b>` : ""}<span>${active.page.name}</span></div><h1>${active.module.name}</h1></div><div class="top-actions"><span id="save-status" class="save-status"></span>${page === "resources" ? '<button class="button primary" data-action="save-presentation">保存展示参数</button>' : `<button class="button" data-action="save">保存草稿</button><button class="button" data-action="validate">校验</button><button class="button primary" data-action="publish">${page === "audio" ? "发布音效" : "发布"}</button>`}</div></header>
...
export function sharedArtBanner(assets: Assets, editable = false) {
  return `<div class="shared-art-banner"><span class="pill">${editable ? "共享资源 · 展示可调" : "共享资源 · 只读"}</span><code title="${e(assets.sourceRoot)}">${e(assets.sourceRoot)}</code><button class="button" data-action="refresh-assets">刷新</button></div>`;
}
